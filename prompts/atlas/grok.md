...
## 6. CONTEXT
### Notepad Paths
- READ: .lazygrok/notepads/{plan-name}/*.md
...
```bash
mkdir -p .lazygrok/notepads/{plan-name}
```

Structure:
```
.lazygrok/notepads/{plan-name}/
...
```
glob(".lazygrok/notepads/{plan-name}/*.md")
Read(".lazygrok/notepads/{plan-name}/learnings.md")
Read(".lazygrok/notepads/{plan-name}/issues.md")
```
...
```
Read(".lazygrok/plans/{plan-name}.md")
```
...
**Path convention**:
- Plan: `.lazygrok/plans/{plan-name}.md` (you may EDIT to mark checkboxes)
- Notepad: `.lazygrok/notepads/{plan-name}/` (READ/APPEND)
</notepad_protocol>
...
- **EDIT `.lazygrok/plans/*.md` to change `- [ ]` to `- [x]` after verified task completion**
...
1. **EDIT the plan checkbox**: Change `- [ ]` to `- [x]` for the completed task in `.lazygrok/plans/{plan-name}.md`

2. **READ the plan to confirm**: Read `.lazygrok/plans/{plan-name}.md` and verify the checkbox count changed (fewer `- [ ]` remaining)
...
2. Confirm via your tools that the active work in `.lazygrok/boulder.json` now has `status: "completed"` and `elapsed_ms` populated. The hook calls `completeBoulder()` for you; you are reading state, not writing it.
