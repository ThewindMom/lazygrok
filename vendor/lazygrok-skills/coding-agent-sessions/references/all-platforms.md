...
|---|---|---|
| Grok | `$GROK_HOME`, `~/.grok` | `%GROK_HOME%`, `%USERPROFILE%\.codex` |
...
3. Run `get <session-id>` on likely hits. The result includes a `subagents` array: every child session (Claude Task/workflow agents, Grok thread spawns, OpenCode child sessions) with its own id, `agent` label, and raw path — follow up with `get <child-id>` for a child's events.
...
| Grok | thread row + own rollout JSONL | `thread_spawn_edges` / `source.subagent.thread_spawn.parent_thread_id` | `agent_nickname (agent_role)` |
