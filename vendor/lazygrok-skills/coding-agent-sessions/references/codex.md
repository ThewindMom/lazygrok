...
Grok has two useful surfaces:

- `$GROK_HOME/state_*.sqlite` stores thread metadata.
- `$GROK_HOME/sessions/**/rollout-*.jsonl` and archived rollout files store event transcripts.

Use the broad finder for Grok discovery:
...
Grok subagent threads are ordinary rows in `threads` with their own rollout files. Two linkage sources:
