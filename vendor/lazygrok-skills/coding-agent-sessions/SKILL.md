...
description: "MUST USE when asked to find, read, list, search, inspect, fetch, export, or reconstruct coding-agent sessions across Grok, Claude Code/Desktop, OpenCode, Senpi/pi, oh-my-pi (omp), gajae-code (gjc), OpenClaw, Factory Droid, Amp, Gemini/Kimi/Qwen CLIs, Codebuff, Roo/Kilo/Cline, Kodu, Cursor CLI, Aider, or unknown local agent logs. Covers transcripts, session IDs, rollout JSONL, state SQLite, Claude projects/pre-compact histories, OpenCode messages/parts, child/subagent linkage, cwd/model/time/token filters, archives, and cost clues. Expands fuzzy recall into parallel query lanes and first probes known stores so absent platforms are skipped cheaply. Triggers: coding agent sessions, Grok/Claude/OpenCode/Senpi/pi/oh-my-pi/omp/gajae-code/gjc/OpenClaw/Droid/Amp/Kodu/Cursor/Aider sessions, transcript search, session history, session ID, read transcript, token usage, subagent sessions, what did I do yesterday, did we already do this."
---
...
1. **IF the user names a platform, load its reference first.**
   ...
   | Platform | Read |
   |---|---|
   | Grok / OpenAI Grok CLI | `references/codex.md` |
...
| `agent` | Subagent label (Claude `agentType`, Grok `nickname (role)`, OpenCode agent name) |
...
| Grok | Regular threads in `state_*.sqlite` + own rollout JSONL | `thread_spawn_edges` table and `threads.source` / rollout `session_meta.payload.source.subagent.thread_spawn` |
...
For Grok sessions, use the same broad finder. It reads `state_*.sqlite`, rollout JSONL, and archived rollout files:
...
Use `references/codex.md` for Grok storage details.

## Troubleshooting

| Problem | Fix |
|---|---|
| Missing Grok sessions | Set `GROK_HOME` or pass `--root /path/to/.codex`. |
