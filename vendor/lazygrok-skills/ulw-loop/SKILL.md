...
- Every `spawn_subagent` prompt starts with `TASK:`, then names `DELIVERABLE`, `SCOPE`, and `VERIFY`; put role and specialty instructions inside `prompt`; use `background: true` unless full history is truly required.
