...
const handlers = {
	...
	async "add-member"(cwd, flags) {
		...
		const delivery = isMultiAgentV2(team)
			? `Send this with spawn_subagent using only task_name "${member.taskName}", message <bootstrap>, and fork_turns "none"; then bind-agent --agent-path "${member.agentPath}"`
	...
	async "worktree-add"(cwd, flags) {
		...
		const followUp = v2
			? member.status === "active"
				? `\nMember agent: ${member.agentPath}\nSend it a spawn_subagent (re-task: new prompt to same role) to: cd "${result.path}"`
				: `\nMember agent is not spawned yet; include this worktree path in its spawn_subagent message, then bind-agent.`
			: member.threadId
				? `\nMember thread: ${codexThreadLink(member.threadId)}\nTell that member to: cd "${result.path}"`
				: "\nMember thread is not bound yet; wait for the real Grok thread id, then bind-thread before sending bootstrap. After binding, send the member this worktree path.";
	...
	async archive(cwd, flags) {
		...
		const teamSummary = v2
			? `archived team ${sessionId}; V2 has no runtime archive operation - the durable team state is the archive (kill_command_or_subagent any member still mid-turn)\n`
			: `archived team ${sessionId} and closed all members\n`;
		const memberSummary = v2
			? `archived member ${flags.id} in team state; V2 has no runtime archive operation (kill_command_or_subagent it if still mid-turn)\n`
