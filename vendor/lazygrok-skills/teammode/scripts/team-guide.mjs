...
function transportLabel(team) {
	return isMultiAgentV2(team)
		? "Grok spawn_subagent (native agents: flat `spawn_subagent` / `spawn_subagent (message-only follow-up)` / `spawn_subagent (re-task: new prompt to same role)`)"
		: "Grok threads (`codex_app.*` tools)";
}
...
as a \`spawn_subagent (re-task: new prompt to same role)\` - switch to it before your next edit.`;
	}
	return `If Grok returns only \`pendingWorktreeId\` while the leader is creating a worktree-backed member
thread, the thread is not ready yet. The leader must wait until Grok surfaces a real thread id,
...
function addressBookRules(team) {
	if (isMultiAgentV2(team)) {
		return `- **Reach the leader and your peers with \`spawn_subagent (message-only follow-up)\` - not by narrating in your own
  ...
  \`members[].taskName\`).
- **Use \`spawn_subagent (message-only follow-up)\` for updates and \`spawn_subagent (re-task: new prompt to same role)\` only when handing a peer a NEW task**
...
- **When you mention a Grok thread, include its app link too:** \`codex://threads/<threadId>\`.
...
export function buildGuide(team) {
	const v2 = isMultiAgentV2(team);
	const artifacts = team.paths?.artifacts ?? ".lazygrok/teams/<session_id>/artifacts";
	const teamJson = team.paths?.team ?? ".lazygrok/teams/<session_id>/team.json";
...
function memberEndpointNote(team, member) {
	...
	return member.threadId ? `\nYour Grok thread link is ${codexThreadLink(member.threadId)}. Include it when reporting handoffs or worktree status.` : "";
}

function pushInstruction(team) {
	if (isMultiAgentV2(team)) {
		return `Push updates to the leader (target \`${LEADER_AGENT_PATH}\`) and relevant peers (their team.json \`members[].agentPath\`) with \`spawn_subagent (message-only follow-up)\` as you work`;
	}
...
export function buildMemberPrompt(team, id) {
	...
	const guide = team.paths?.guide ?? ".lazygrok/teams/<session_id>/guide.md";
	const teamJson = team.paths?.team ?? ".lazygrok/teams/<session_id>/team.json";
	...
	const readiness =
		team.worktree?.enabled
			? "\nBefore editing, verify that your assigned worktree exists and contains repo files. If this prompt arrived before your Grok worktree checkout is ready, or the path is missing, empty, or not a git worktree/repository yet, report `BLOCKED: worktree not ready` to the leader and wait."
			: "";
