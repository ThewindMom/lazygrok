---
name: teammode
description: "Grok-only team orchestration: run a named team of cooperating Grok workers with durable, script-managed state. MUST USE when the user asks Grok to create, run, coordinate, inspect, archive, or delete a team of agents/threads/sessions, or to work on something as a team in parallel. FIRST inspects the active tool surface (checking tool_search for deferred tools) and tells the user the route: native Grok spawn_subagent agents (flat spawn_subagent with task_name) when available, Grok threads as the fallback, or a plain-subagent split when neither set exists. The main session is always the leader; members are defined by a concrete part, ownership area, or perspective - never a vague job role; a bundled cross-platform script writes the .lazygrok/teams state plus an auto-generated member field manual. Use a team when the work is not perfectly isolated but parallelizing helps; use plain subagents when scope is perfectly isolated or the goal is ambiguous. Triggers: team mode, teammode, make a team, run as a team, team of agents, coordinate threads, parallel Grok threads, archive the team."
---

> **Grok: multi_agent_v2 / team mode transport is n/a.** Use `spawn_subagent` + LazyGrok agents instead. Thread-title hooks are no-ops without Codex create_thread.
...
Use plain fire-and-forget subagents (`$ulw` / one-off `spawn_subagent` workers) - NOT a team - when
EITHER holds:
...
1. **Grok spawn_subagent (preferred)** - select when the flat V2 collaboration tools are ALL active:
   flat `spawn_subagent` whose schema requires `task_name`, plus `spawn_subagent (message-only follow-up)`, `spawn_subagent (re-task: new prompt to same role)`,
   `get_command_or_subagent_output`, `list_agents`, and `kill_command_or_subagent`. Members are durable native agents
   addressed by task name / agent path (`/root/<task_name>`). The namespaced
   `spawn_subagent` surface never qualifies as a team transport.
2. **Grok threads (fallback)** - select when flat V2 is not available but the
...
3. **Neither set visible** - if a `tool_search` tool is active, search for the missing sets
   (e.g. `spawn_subagent`, `codex_app`) before concluding: some environments defer tools behind
...
- `Teammode transport: Grok spawn_subagent (flat spawn_subagent with task_name).`
- `Teammode transport: Grok threads (flat V2 tools not present in this session).`
- `Teammode unavailable: neither Grok spawn_subagent nor codex_app tools exist in this session -
  using <visible plain-subagent mechanism> for independent scopes.`
- `Teammode unavailable: neither Grok spawn_subagent nor codex_app tools exist in this session, and
  no compatible plain-subagent mechanism is available - continuing serially.`

Pass the choice to `init` as `--transport spawn_subagent` or `--transport codex_app`. The
transport is recorded in `team.json` and is IMMUTABLE for the team's lifetime: a V2 spawn
failure is a V2 blocker to report, never permission to mix Grok threads into the same
...
one name for two members. On Grok spawn_subagent teams also give each member a unique
...
```
node "<skill-root>/scripts/team.mjs" init        --name "<team>" --session-name "<session>" --transport spawn_subagent|codex_app [--session <leader_thread_id>] [--worktree] [--base-branch dev]
...
node "<skill-root>/scripts/team.mjs" bind-agent   --team <session_id> --id A --agent-path /root/<task_name> [--cwd <path>]   # spawn_subagent teams
...
exchange space). On codex_app teams `{session_id}` is the leader's Grok session id when you can
...
command finishes, or inspect `.lazygrok/teams/{session_id}/.team.lock/owner.json` if the previous
command crashed. `bind-agent` refuses codex_app teams and `bind-thread` refuses spawn_subagent
...
**Grok spawn_subagent teams:**
1. If a member needs an isolated worktree, run `worktree-add` BEFORE spawning it - flat
   `spawn_subagent` has no cwd argument, so the path must ride in the bootstrap message.
2. Spawn each member with flat `spawn_subagent` using only the V2 schema fields:
...
3. `bind-agent --agent-path` with the canonical task name the spawn returned (normally
   ...
   and are re-tasked with `spawn_subagent (re-task: new prompt to same role)` - never respawned under a second name.
...
1. Create a durable thread per member with `codex_app.create_thread` - ALWAYS this tool for
   ...
   land at creation. If Grok returns only `pendingWorktreeId`, the worktree-backed thread is
   not ready yet: do not `bind-thread` and do not send the member bootstrap. Wait until Grok
...
- **Grok spawn_subagent:** members reach you with `spawn_subagent (message-only follow-up)` to `/root` and reach peers by their
  `members[].agentPath`. You reach members the same way; use `spawn_subagent (re-task: new prompt to same role)` when you hand an
  idle member NEW work (it wakes the member), `spawn_subagent (message-only follow-up)` for context that should not
  interrupt, and `get_command_or_subagent_output` only when you are genuinely blocked on their next update - a
  `get_command_or_subagent_output` timeout only means no new mailbox update arrived, never that a member failed.
  Your own session IS `/root` - members can always reach you; leave `--session` unset.
- **Grok:** members push with `codex_app.send_message_to_thread`; you inspect state with
...
Delivering the path differs by transport: on Grok spawn_subagent, create the worktree BEFORE spawning
so the bootstrap carries it, or send it to an already-running member as a `spawn_subagent (re-task: new prompt to same role)`; on
Grok, send a follow-up message that includes both the worktree path and the member's
...
- `archive` closes the team: notify each active member, copy anything useful into `artifacts/`,
  then close each member on its transport. On Grok spawn_subagent, `kill_command_or_subagent` any member still
  mid-turn and record in the note that V2 exposes no runtime archive operation - the durable
  `team.json` state IS the archive; never claim a V2 agent itself was archived. On Grok,
  try `codex_app.set_thread_archived` per member thread; treat failures such as
  "Ambiguous Grok thread id" or an id that is ambiguous across hosts as an
...
- Stop if the selected transport's tools (V2 spawn/message/wait/list/interrupt, or Grok
