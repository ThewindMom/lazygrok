...
Grok-adapted plugin component for durable repo-native multi-goal orchestration with embedded success criteria and observable evidence audit. State lives under `.lazygrok/ulw-loop/` and is mutated through the `ulw-loop` CLI.
...
| Subcommand | Purpose |
|------------|---------|
| `ulw-loop help` | Print CLI usage. |
| `ulw-loop create-goals` | Create repo-native goals and seed success criteria from a brief; optionally define review-boundary validation batches with `--validation-batch-json`. |
| `ulw-loop status` | Report active goal, criteria, and evidence state. |
| `ulw-loop complete-goals` | Manual fallback to start or resume the next eligible goal, or report aggregate completion / blocked handoff. |
| `ulw-loop checkpoint` | Gate a goal transition with evidence; complete checkpoints auto-start the next eligible goal by default, with `--no-advance` preserving the legacy two-call flow. |
| `ulw-loop steer` | Apply one steering mutation proposal or an atomic all-or-nothing batch with `--proposals-json`. |
| `ulw-loop add-goal` | Append a goal to the active plan. |
| `ulw-loop criteria` | Inspect one goal's success criteria. |
| `ulw-loop record-evidence` | Record observable evidence for one criterion. |
| `ulw-loop record-review-blockers` | Mark a goal as review-blocked and add follow-up work from final-review findings. |
| `ulw-loop light-quality-gate` | Write an honest root self-review gate for LIGHT-tier work with explicit provenance. |
