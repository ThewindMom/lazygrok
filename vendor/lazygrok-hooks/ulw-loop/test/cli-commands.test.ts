...
describe("ulwLoopCommand help", () => {
	it("prints usage when no subcommand", async () => {
		...
		expect(out.join("")).toContain("ulw-loop");
	});
});
...
describe("ulwLoopCommand error handling", () => {
	it("#given a noncanonical explicit session id #when creating a plan #then rejects it instead of aliasing state", async () => {
		expect(
			await ulwLoopCommand([
				"create-goals",
				"--brief",
				"Do not alias hostile sessions",
				"--session-id",
				"../safe-session",
				"--json",
			]),
		).toBe(1);
		expect(stdoutJson()).toMatchObject({ ok: false, error: { code: "ULW_LOOP_SESSION_ID_INVALID" } });
	});
