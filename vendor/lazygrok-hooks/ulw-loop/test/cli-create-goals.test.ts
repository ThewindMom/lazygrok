...
let originalGrokSessionId: string | undefined;
let originalGrokThreadId: string | undefined;
...
beforeEach(async () => {
	...
	originalGrokSessionId = process.env["GROK_SESSION_ID"];
	originalGrokThreadId = process.env["GROK_THREAD_ID"];
...
afterEach(async () => {
	...
	if (originalGrokSessionId === undefined) delete process.env["GROK_SESSION_ID"];
	else process.env["GROK_SESSION_ID"] = originalGrokSessionId;
	if (originalGrokThreadId === undefined) delete process.env["GROK_THREAD_ID"];
	else process.env["GROK_THREAD_ID"] = originalGrokThreadId;
...
describe("ulwLoopCommand create-goals", () => {
	it("creates plan + writes 3 artifacts + seeds criteria per goal", async () => {
		...
		expect(await readFile(join(testDir, ".lazygrok/ulw-loop/brief.md"), "utf8")).toContain("Goal A");
		expect(await readFile(join(testDir, ".lazygrok/ulw-loop/goals.json"), "utf8")).toContain("successCriteria");
		expect(await readFile(join(testDir, ".lazygrok/ulw-loop/ledger.jsonl"), "utf8")).toContain("plan_created");
	});
	...
	it("#given completed default aggregate #when creating another default plan #then guides to a fresh session", async () => {
		...
		const checkpointCode = await ulwLoopCommand([
			"checkpoint",
			"--goal-id",
			"G001-finished",
			"--status",
		...
		expect(checkpointCode, err.join("")).toBe(0);
		resetOutput();
	...
	it("#given an active goal on a v2 plan #when reading status --json #then exposes the current attempt dir", async () => {
		...
		expect(stdoutJson()).toMatchObject({ currentAttemptDir: ".lazygrok/evidence/ulw/session/G001-goal-a/a1" });
	});
	...
	it("#given two session ids #when creating goals #then writes isolated session-scoped plans", async () => {
		...
		expect(await readFile(join(testDir, ".lazygrok/ulw-loop/session-A/goals.json"), "utf8")).toContain("Alpha");
		expect(await readFile(join(testDir, ".lazygrok/ulw-loop/session-B/goals.json"), "utf8")).toContain("Beta");

		expect(await ulwLoopCommand(["status", "--session-id", "session-A", "--json"])).toBe(0);
		expect(stdoutJson()).toMatchObject({
			plan: { goalsPath: ".lazygrok/ulw-loop/session-A/goals.json", goals: [{ title: "Alpha" }] },
		});
		expect(out.join("")).not.toContain("Beta");
	});

	it("#given Grok session env #when creating goals #then uses the session scope", async () => {
		process.env["GROK_SESSION_ID"] = "session-123";

		expect(await ulwLoopCommand(["create-goals", "--brief", "- Thread scoped", "--json"])).toBe(0);
		resetOutput();

		expect(await readFile(join(testDir, ".lazygrok/ulw-loop/session-123/goals.json"), "utf8")).toContain(
			"Thread scoped",
		);
		expect(await ulwLoopCommand(["status", "--json"])).toBe(0);
		expect(stdoutJson()).toHaveProperty("plan.goalsPath", ".lazygrok/ulw-loop/session-123/goals.json");
	});

	it("#given Grok session env and explicit session id #when creating goals #then the explicit session wins", async () => {
		process.env["GROK_SESSION_ID"] = "session-123";
		...
		expect(await readFile(join(testDir, ".lazygrok/ulw-loop/manual-456/goals.json"), "utf8")).toContain(
			"Manual scoped",
		);
	});

	it("#given only an ambient Codex session env #when creating goals #then LazyGrok ignores it", async () => {
		process.env["CODEX_SESSION_ID"] = "parent-codex-session";

		expect(await ulwLoopCommand(["create-goals", "--brief", "- Grok scoped", "--json"])).toBe(0);

		expect(await readFile(join(testDir, ".lazygrok/ulw-loop/goals.json"), "utf8")).toContain("Grok scoped");
		await expect(
			readFile(join(testDir, ".lazygrok/ulw-loop/parent-codex-session/goals.json"), "utf8"),
		).rejects.toThrow();
	});
	...
	it("#given a valueless --session-id #when creating goals #then it fails and writes no plan", async () => {
		...
		await expect(readFile(join(testDir, ".lazygrok/ulw-loop/goals.json"), "utf8")).rejects.toThrow();
	});
