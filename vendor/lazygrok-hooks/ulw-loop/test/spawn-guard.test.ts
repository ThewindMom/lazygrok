...
describe("applySpawnGuards fan-out cap", () => {
	...
	it("#given the canonical state root is a symlink #when guarded #then does not write outside", async () => {
		const outside = await mkdtemp(join(tmpdir(), "ulw-spawn-outside-"));
		try {
			mkdirSync(join(outside, "ulw-loop", "s1"), { recursive: true });
			writeFileSync(join(outside, "ulw-loop", "s1", "goals.json"), "{}");
			symlinkSync(outside, join(workDir, ".lazygrok"), platform === "win32" ? "junction" : "dir");

			expect(deny(applySpawnGuards(payload("spawn_agent", { message: "scan" }))).permissionDecision).toBe("deny");
			expect(existsSync(join(outside, "ulw-loop", "s1", "spawn-count.json"))).toBe(false);
		} finally {
			await rm(outside, { recursive: true, force: true });
		}
	});

	it("#given the legacy state root is a symlink #when guarded #then does not write outside", async () => {
		const outside = await mkdtemp(join(tmpdir(), "ulw-spawn-outside-"));
		try {
			mkdirSync(join(outside, "ulw-loop", "s1"), { recursive: true });
			writeFileSync(join(outside, "ulw-loop", "s1", "goals.json"), "{}");
			symlinkSync(outside, join(workDir, ".omo"), platform === "win32" ? "junction" : "dir");
	...
	it.each([
		"spawn_subagent",
		"task",
	])("#given live Grok spawn token %s #when guarded #then it reserves the ULW budget", (toolName: string) => {
		writeGoals();
		process.env["OMO_SPAWN_FANOUT_LIMIT"] = "1";

		expect(applySpawnGuards(payload(toolName, { message: "scan" }))).toBe("");
		const second = applySpawnGuards(payload(toolName, { message: "scan" }));
