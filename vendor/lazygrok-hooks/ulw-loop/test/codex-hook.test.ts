...
describe("runUlwLoopHookCli (stdin/stdout integration)", () => {
	...
	it("writes nothing when stdin exceeds 10 MiB", async () => {
		const capture = captureStdout();
		await runUlwLoopHookCli(Readable.from([Buffer.alloc(10 * 1024 * 1024 + 1)]), capture.stdout);
		expect(capture.read()).toBe("");
	});
});
...
describe("runPreToolUseGoalBudgetGuardCli", () => {
	...
	it("#given oversized PreToolUse stdin #when CLI hook runs #then it stays silent", async () => {
		const capture = captureStdout();
		await runPreToolUseGoalBudgetGuardCli(Readable.from([Buffer.alloc(10 * 1024 * 1024 + 1)]), capture.stdout);
		expect(capture.read()).toBe("");
	});
});
