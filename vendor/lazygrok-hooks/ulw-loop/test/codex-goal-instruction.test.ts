...
describe("buildCodexGoalInstruction aggregate mode", () => {
	...
	it("#given a scoped plan #when rendering final commands #then includes the session id option", () => {
		...
		const scopedCommands = text
			.split("\n")
			.filter((line) => line.includes("--session-id"))
			.flatMap((line) =>
				[...line.matchAll(/\b(checkpoint|complete-goals|record-review-blockers)\b/gu)].map((match) => match[1]),
			);
		expect(new Set(scopedCommands)).toEqual(new Set(["checkpoint", "complete-goals", "record-review-blockers"]));
		const scopedSessionIds = [...text.matchAll(/--session-id\s+(\S+)/gu)].map((match) => match[1]);
		expect(scopedSessionIds.length).toBeGreaterThan(0);
		expect(scopedSessionIds.every((sessionId) => sessionId === "session-A")).toBe(true);
	});
});
