...
describe("readCodexGoalSnapshotInput", () => {
	...
	it("reads from file path", async () => {
		...
		// when
		const snapshot = await readCodexGoalSnapshotInput(filePath, dir);
	...
	it("rejects a snapshot path outside the workspace root", async () => {
		const workspace = join(dir, "workspace");
		const outside = join(dir, "outside.json");
		await import("node:fs/promises").then((fs) => fs.mkdir(workspace));
		await writeFile(outside, '{"goal":{"objective":"outside","status":"complete"}}', "utf8");

		await expect(readCodexGoalSnapshotInput(outside, workspace)).rejects.toThrow(CodexGoalSnapshotError);
	});
});
