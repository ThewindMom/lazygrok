...
import { type ValidateQualityGateOptions, validateQualityGate } from "../src/quality-gate.js";
...
const FS_OPTS = {
	...
	fs: { existsSync, lstatSync, realpathSync, statSync },
} as const;
...
describe("validateQualityGate", () => {
	it("#given explicit root self-review provenance #when all reviewer fields name the root #then it is accepted honestly", () => {
		const gate = makeGate({
			provenance: {
			...
			codeReview: { ...VALID_GATE.codeReview, by: "lazygrok-root" },
			manualQa: { ...VALID_GATE.manualQa, by: "lazygrok-root" },
			gateReview: { ...VALID_GATE.gateReview, by: "lazygrok-root" },
		});

		expect(validateQualityGate(gate)).toMatchObject({
			provenance: { mode: "root-self-review", producer: "lazygrok-root" },
			codeReview: { by: "lazygrok-root" },
			manualQa: { by: "lazygrok-root" },
			gateReview: { by: "lazygrok-root" },
		});
	});

	it("#given a root reviewer identity without root provenance #when validated #then it fails closed", () => {
	...
	it("#given root provenance with a fabricated specialist identity #when validated #then it fails closed", () => {
		const error = getQualityGateError(
			makeGate({
				provenance: {
					mode: "root-self-review",
		...
		expect(error.message).toContain("codeReview.by");
		expect(error.message).toContain("lazygrok-root");
	});
...
describe("validateQualityGate attempt containment", () => {
	...
	it("#given a symlinked current-attempt artifact #when validating #then the gate rejects it", () => {
		const artifactPath = resolve(COMPONENT_ROOT, "test/fixtures/artifacts/cli-pass.txt");
		const opts = {
			...ATTEMPT_OPTS,
			fs: {
				...ATTEMPT_OPTS.fs,
				lstatSync(path: string) {
					if (path === artifactPath) return { isSymbolicLink: () => true };
					return lstatSync(path);
				},
			},
		} as const;
		expect(() => validateQualityGate(makeGate(), opts)).toThrow(/symbolic link/);
	});

	it("#given a current-attempt artifact hard linked to another file #when validating #then the gate rejects it", () => {
		const fixture = mkdtempSync(join(COMPONENT_ROOT, "test/quality-gate-hardlink-"));
		const outside = `${fixture}-external.txt`;
		try {
			const gate = gateForAttempt(fixture);
			writeFileSync(outside, "external\n");
			rmSync(join(fixture, "cli-pass.txt"));
			linkSync(outside, join(fixture, "cli-pass.txt"));
			expect(() => validateQualityGate(gate, attemptOptions(fixture))).toThrow(/single-link regular artifact/);
		} finally {
	...
	it("#given a current-attempt artifact is a directory #when validating #then the gate rejects it", () => {
		const fixture = mkdtempSync(join(COMPONENT_ROOT, "test/quality-gate-directory-"));
		try {
			const gate = gateForAttempt(fixture);
			rmSync(join(fixture, "cli-pass.txt"));
			mkdirSync(join(fixture, "cli-pass.txt"));
			expect(() => validateQualityGate(gate, attemptOptions(fixture))).toThrow(/single-link regular artifact/);
		} finally {
			rmSync(fixture, { recursive: true, force: true });
		}
...
function gateForAttempt(attempt: string): Record<string, unknown> {
	for (const name of ["code-review.md", "gate-review.md", "cli-pass.txt", "rejection.txt"]) {
		writeFileSync(join(attempt, name), `${name}\n`);
	}
	const path = (name: string) => relative(COMPONENT_ROOT, join(attempt, name));
	return {
		...VALID_GATE,
		codeReview: { ...VALID_GATE.codeReview, reportPath: path("code-review.md") },
		gateReview: { ...VALID_GATE.gateReview, reportPath: path("gate-review.md") },
		manualQa: {
			...VALID_GATE.manualQa,
			artifactRefs: [
				{ ...VALID_GATE.manualQa.artifactRefs[0], path: path("cli-pass.txt") },
				{ ...VALID_GATE.manualQa.artifactRefs[1], path: path("rejection.txt") },
			],
		},
	};
}

function attemptOptions(attempt: string): ValidateQualityGateOptions {
	return {
		...FS_OPTS,
		currentAttemptDir: relative(COMPONENT_ROOT, attempt),
	};
}
