...
const STABLE_OBJECTIVE =
	"Complete the durable ulw-loop plan in .lazygrok/ulw-loop/goals.json, including later accepted/appended stories, under the original brief constraints; use .lazygrok/ulw-loop/ledger.jsonl as the audit trail.";
...
describe("readUlwLoopPlan", () => {
	...
	it("throws UlwLoopError when goals.json is missing", async () => {
		...
		await expect(readUlwLoopPlan(repoRoot)).rejects.toThrow("ulw-loop create-goals");
	});
	...
	it("rejects a plan whose goal id is not one safe path segment", async () => {
		await writeRawPlan(repoRoot, makePlan({ goals: [makeGoal({ id: "../../../escaped" })] }));

		await expect(readUlwLoopPlan(repoRoot)).rejects.toMatchObject({
			code: "ULW_LOOP_UNSAFE_PATH",
		});
	});

	it("keeps an existing legacy .omo run on its original state root", async () => {
		// given
		const legacyDir = join(repoRoot, ".omo", "ulw-loop");
		await mkdir(legacyDir, { recursive: true });
		await writeFile(join(legacyDir, "goals.json"), `${JSON.stringify(makePlan(), null, 2)}\n`, "utf8");

		// when
		const plan = await readUlwLoopPlan(repoRoot);
		await writePlan(repoRoot, { ...plan, updatedAt: "2026-05-24T00:00:00.000Z" });

		// then
		expect(ulwLoopDir(repoRoot)).toBe(legacyDir);
		expect(JSON.parse(await readFile(join(legacyDir, "goals.json"), "utf8"))).toMatchObject({
...
describe("writePlan", () => {
	...
	it("overwrites existing file", async () => {
		...
		const before = await stat(ulwLoopGoalsPath(repoRoot));

		// when
		await writePlan(repoRoot, makePlan({ codexObjective: "second" }));
		const after = await stat(ulwLoopGoalsPath(repoRoot));
	...
	it("repairs private state permissions while writing", async () => {
		const repoRoot = await makeRepo();
		await mkdir(ulwLoopDir(repoRoot), { recursive: true });
		await chmod(join(repoRoot, ".lazygrok"), 0o755);
		await chmod(ulwLoopDir(repoRoot), 0o755);

		await writePlan(repoRoot, makePlan());

		for (const directory of [join(repoRoot, ".lazygrok"), ulwLoopDir(repoRoot)]) {
			expect((await stat(directory)).mode & 0o777).toBe(0o700);
		}
		expect((await stat(ulwLoopGoalsPath(repoRoot))).mode & 0o777).toBe(0o600);
	});

	it("rejects a hard-linked goals file without modifying the other link", async () => {
		const repoRoot = await makeRepo();
		const outside = await makeRepo();
		await writePlan(repoRoot, makePlan({ codexObjective: "inside" }));
		const goalsPath = ulwLoopGoalsPath(repoRoot);
		const outsideTarget = join(outside, "outside.json");
		const original = await readFile(goalsPath, "utf8");
		await writeFile(outsideTarget, original, "utf8");
		await unlink(goalsPath);
		await link(outsideTarget, goalsPath);

		await expect(writePlan(repoRoot, makePlan({ codexObjective: "changed" }))).rejects.toMatchObject({
			code: "ULW_LOOP_UNSAFE_PATH",
		});
		await expect(readFile(outsideTarget, "utf8")).resolves.toBe(original);
	});

	it("rejects a symlinked state root instead of writing outside the workspace", async () => {
		const repoRoot = await makeRepo();
		const outside = await makeRepo();
		await symlink(outside, join(repoRoot, ".lazygrok"), "dir");

		await expect(writePlan(repoRoot, makePlan())).rejects.toMatchObject({
			code: "ULW_LOOP_UNSAFE_PATH",
		});
		await expect(readdir(outside)).resolves.toEqual([]);
	});

	it("rejects create-goals through a symlinked state root before writing the brief", async () => {
		const repoRoot = await makeRepo();
		const outside = await makeRepo();
		await symlink(outside, join(repoRoot, ".lazygrok"), "dir");

		await expect(createUlwLoopPlan(repoRoot, { brief: "- Keep state confined" })).rejects.toMatchObject({
			code: "ULW_LOOP_UNSAFE_PATH",
		});
		await expect(readdir(outside)).resolves.toEqual([]);
	});

	it("rejects force create-goals through a legacy symlink without changing the target", async () => {
		const repoRoot = await makeRepo();
		const outside = await makeRepo();
		await mkdir(join(outside, "ulw-loop"), { recursive: true });
		await writeFile(join(outside, "ulw-loop", "goals.json"), "{}\n", "utf8");
		await symlink(outside, join(repoRoot, ".omo"), "dir");

		await expect(createUlwLoopPlan(repoRoot, { brief: "PWN", force: true })).rejects.toMatchObject({
			code: "ULW_LOOP_UNSAFE_PATH",
		});
		await expect(readdir(join(outside, "ulw-loop"))).resolves.toEqual(["goals.json"]);
	});
});
...
describe("readSteeringLedgerEntries", () => {
	...
	it("rejects an oversized ledger line before parsing or collecting it", async () => {
		const repoRoot = await makeRepo();
		await mkdir(ulwLoopDir(repoRoot), { recursive: true });
		await writeFile(ulwLoopLedgerPath(repoRoot), `${"x".repeat(1024 * 1024 + 1)}\n`, "utf8");

		await expect(readSteeringLedgerEntries(repoRoot)).rejects.toMatchObject({
			code: "ULW_LOOP_LEDGER_TOO_LARGE",
		});
	});
});
