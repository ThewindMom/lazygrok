...
describe("steerUlwLoopBatch", () => {
	...
	it("#given mixed fresh and deduped proposals #when accepted and replayed #then records fresh audits and replay records none", async () => {
		const repo = await repoWithPlan();
		...
		const ledgerBeforeReplay = await readSteeringLedgerEntries(repo);
		...
		expect(await readSteeringLedgerEntries(repo)).toEqual(ledgerBeforeReplay);
	});
});
