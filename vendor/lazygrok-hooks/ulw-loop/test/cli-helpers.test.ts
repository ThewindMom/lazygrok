...
describe("readJsonInput", () => {
	...
	it("reads from file path", async () => {
		...
		try {
			...
			await expect(readJsonInput(file, dir)).resolves.toEqual({ fromFile: true });
		} finally {
			await rm(dir, { recursive: true, force: true });
		}
	});

	it("rejects a file path outside the workspace root", async () => {
		const dir = await mkdtemp(join(tmpdir(), "ug-cli-json-"));
		const workspace = join(dir, "workspace");
		const outside = join(dir, "outside.json");
		try {
			await import("node:fs/promises").then((fs) => fs.mkdir(workspace));
			await writeFile(outside, JSON.stringify({ outside: true }), "utf8");

			await expect(readJsonInput(outside, workspace)).rejects.toThrow(UlwLoopError);
		} finally {
			await rm(dir, { recursive: true, force: true });
		}
	});
...
describe("ULW_LOOP_HELP", () => {
	it("mentions the Grok-compatible ulw-loop entrypoint + every subcommand", () => {
		expect(ULW_LOOP_HELP).toContain("ulw-loop");
