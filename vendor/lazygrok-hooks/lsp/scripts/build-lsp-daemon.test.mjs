...
test("#given daemon source is newer than dist #when bootstrapping component #then daemon rebuild runs", async () => {
	const fixture = await makeFixture();
	const fakeBin = join(fixture.root, "bin");
	const npmLog = join(fixture.root, "npm.log");
	await mkdir(join(fixture.packageDir, "src"), { recursive: true });
	await new Promise((resolve) => setTimeout(resolve, 20));
	await writeFile(join(fixture.packageDir, "src", "index.ts"), "export {};\n");
	await mkdir(fakeBin, { recursive: true });
	await writeFile(
		join(fakeBin, "npm"),
		`#!/usr/bin/env node\nrequire("node:fs").appendFileSync(${JSON.stringify(npmLog)}, process.argv.slice(2).join(" ") + "\\n");\n`,
	);
	await chmod(join(fakeBin, "npm"), 0o755);
