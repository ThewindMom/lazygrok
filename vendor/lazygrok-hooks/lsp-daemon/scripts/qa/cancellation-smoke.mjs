...
const daemonSourceRoot = existsSync(join(root, "packages/lsp-daemon/src"))
	? join(root, "packages/lsp-daemon")
	: root;
const coreSourceRoot = existsSync(join(root, "packages/lsp-core/src"))
	? join(root, "packages/lsp-core")
	: join(root, "..", "lsp-core");

const { callToolViaDaemon } = await import(pathToFileURL(join(daemonSourceRoot, "src/daemon-client.ts")).href);
const { daemonPaths: resolveDaemonPaths, OMO_LSP_DAEMON_DIR } = await import(
	pathToFileURL(join(daemonSourceRoot, "src/paths.ts")).href
);
const { handleDaemonMessage } = await import(pathToFileURL(join(daemonSourceRoot, "src/request-routing.ts")).href);
const { createLineDecoder, encodeJsonLine } = await import(
	pathToFileURL(join(daemonSourceRoot, "src/socket-jsonrpc.ts")).href
);
const { JsonRpcConnection } = await import(
	pathToFileURL(join(coreSourceRoot, "src/lsp/json-rpc-connection.ts")).href
);
const { disposeDefaultLspManager } = await import(pathToFileURL(join(coreSourceRoot, "src/lsp/manager.ts")).href);
...
try {
	const paths = resolveDaemonPaths(
		...
		{ version: "manual", cliPath: join(daemonSourceRoot, "src/cli.ts") },
	);
