...
describe("daemon IPC authentication and ownership", () => {
	...
	it("given two authenticated contexts when routed through one daemon then both existing cwd scopes are honored", async () => {
		...
		const first = await callToolViaDaemon(
			"status",
			{},
			{ paths, ensure: async () => {}, context: existingContext(firstRoot) },
		);
		const second = await callToolViaDaemon(
			"status",
			{},
			{ paths, ensure: async () => {}, context: existingContext(secondRoot) },
		);
	...
	it("given live unreachable owner metadata when candidate starts then it defers without unlinking endpoint", async () => {
		...
		writeFileSync(
			paths.owner,
			JSON.stringify({ pid: process.pid, nonce: "live", startedAt: "now", endpoint: { path: paths.socket } }),
			{
				mode: 0o600,
			},
		);
	...
	it("given a dead unchanged owner when candidate starts then it rotates auth and takes ownership", async () => {
		...
		writeFileSync(
			paths.owner,
			JSON.stringify({ pid: 9_999_999, nonce: "dead", startedAt: "old", endpoint: { path: paths.socket } }),
			{
				mode: 0o600,
			},
		);
