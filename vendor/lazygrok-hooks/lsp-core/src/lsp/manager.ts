import { LspRequestContextUnavailableError, lspRequestContext } from "../request-context.js";
...
export class LspManager {
	...
	private getKey(root: string, server: ResolvedServer): string {
	...
	releaseClient(root: string, server: ResolvedServer): void {
		const key = this.getKey(root, server);
	...
	invalidateClient(root: string, server: ResolvedServer, client?: LspClient): void {
		const key = this.getKey(root, server);
		const managed = this.clients.get(key);
		if (!managed) return;
	...
	isServerInitializing(root: string, server: ResolvedServer): boolean {
	...
	hasClient(root: string, server: ResolvedServer): boolean {
...
function requestContextIdentity(): unknown {
	try {
		...
		if (error instanceof LspRequestContextUnavailableError) return null;
		throw error;
	}
}

function serverIdentity(server: ResolvedServer): unknown {
	return [
		server.id,
		server.command,
		server.extensions,
