...
export type ReadonlyFileSystem = {
	readBoundedRegularTextFile?(path: string, maxBytes: number): string;
	lstatSync?(path: string): { readonly isSymbolicLink: () => boolean };
	readFileSync(path: string, encoding: "utf8"): string;
	statSync?(path: string): { readonly isFile: () => boolean; readonly size: number };
};
