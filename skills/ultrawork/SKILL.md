---
name: ultrawork
description: Binding ultrawork mode directive for lazygrok on Grok. When a prompt contains ultrawork or ulw, the lazygrok UserPromptSubmit hook injects a short bootstrap that points at this file. Read the whole file and follow every rule in it for the rest of the task.
metadata:
...
no context fork and keep doing root work while they run.
...
transition, `update_goal` continuation, implementation tool call, plan
...
1. Spawn a child with no context fork and a self-contained reviewer
   assignment in `message`. Pass the reviewer requirements into
   the message.
