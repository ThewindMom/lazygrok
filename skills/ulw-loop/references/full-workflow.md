...
3a. Spawn lazygrok-code-reviewer and lazygrok-qa-executor in parallel (`background: true` on v1; `background: true` on v2) with brief, goals, desired outcome, diff, evidence; wait for BOTH and confirm their report artifacts exist on disk.
3b. Only then spawn lazygrok-gate-reviewer with those artifact paths.
