...
## Goal
Deliver every goal in `.lazygrok/ulw-loop/goals.json` end-to-end.
...
Audit each pass, fail, block, steering change, and checkpoint in `.lazygrok/ulw-loop/ledger.jsonl`.
...
3. **Browser use** — in Grok, use `playwright MCP tools` first when available and the scenario does not need an authenticated or persistent user browser profile. Otherwise use Chrome to drive the REAL page; if unavailable, use agent-browser. Capture action log + screenshot path. Never downgrade a browser-facing criterion.
...
| Implementation — pick the tier by change SIZE: LOW small (one-file fix, boilerplate) / MEDIUM mid-sized (standard feature, a few files) / HIGH large (new module, cross-module, concurrency/security/migration, or a big complex problem with one clear goal) | `TASK: act as a <low|medium|high>-difficulty implementation worker. ...` + `subagent_type: "lazygrok:lazygrok-worker-<low|medium|high>"` when exposed |
...
- Start every `spawn_subagent` message with `TASK: <imperative assignment>`, then name `DELIVERABLE`, `SCOPE`, and `VERIFY`. State that it is an executable assignment, not a context handoff.
- Use `background: true` (v1: `background: true`) unless full history is truly required; paste only the context the child needs. Full-history forks can make the child continue old parent context instead of the delegated task.
...
- Fallback only when the child is completed without the deliverable, ack-only after `spawn_subagent (re-task: new prompt to same role)`, explicitly `BLOCKED:`, or no longer running. Then send `TASK STILL ACTIVE: return <deliverable> or BLOCKED: <reason>` via `spawn_subagent (re-task: new prompt to same role)` when it can still recover the lane; otherwise record inconclusive, do not count it as pass/review approval, stop it if safe, and respawn a smaller `background: true` task with the missing deliverable.

## Artifacts
- `.lazygrok/ulw-loop/brief.md`: original brief and durable constraints.
- `.lazygrok/ulw-loop/goals.json`: goals with embedded `successCriteria` per goal.
- `.lazygrok/ulw-loop/ledger.jsonl`: append-only audit trail.
...
- Never invent state outside `.lazygrok/ulw-loop` artifacts or `omo ulw-loop status --json`.
...
Resolve the CLI before the first command. Prefer `node "$GROK_PLUGIN_ROOT/vendor/lazygrok-hooks/ulw-loop/dist/cli.js"` (alias as `ulw`). `omo ulw-loop` works when available. If `omo` is absent from PATH or lacks `ulw-loop`, use the stable local installer bin or cached Grok component CLI — same CLI, so PATH absence is not a blocker. If PATH is empty, the fallback uses shell builtins and absolute Node locations before reporting guidance, recording the failure in `.lazygrok/ulw-loop/bootstrap-notepad.md`.
```sh
...
if [ -z "${ULW_LOOP_CLI:-}" ]; then
  ...
  printf '%s\n' "Install with grok plugin install github:ThewindMom/lazygrok --trust or set CODEX_LOCAL_BIN_DIR to a PATH directory." >&2
fi
```
...
First survey available skills: read every loosely-relevant skill's description, deliberately choose which this work uses, and prefer applying genuinely-relevant skills over working raw. Load skill bodies only via `read_file` on **absolute** catalog paths under `GROK_PLUGIN_ROOT` / `~/.grok/installed-plugins/lazygrok-*` — never workspace-relative `skills/<name>` for LazyGrok plugin skills.
...
Read pending goals, criteria IDs, current ledger head, blockers, and aggregate Grok objective.
...
2. Inspect host goal state if `get_goal` exists; otherwise use `ulw status --json` / goals.json.
3. Apply this table exactly:

| get_goal result | action |
|-----------------|--------|
| no active goal | Register via Grok goal protocol (`# Goal` + `ulw create-goals`; host `create_goal` only if present). |
...
5. Never create a second Grok goal for the same aggregate objective.
...
6. Treat `.lazygrok/ulw-loop/ledger.jsonl` as the durable audit trail; checkpoint after every success or failure.
7. Per-story Grok goal mode is opt-in only with `--codex-goal-mode per-story`; default is aggregate.
...
11. After completing an aggregate ulw-loop run, clear the Grok goal manually with `/goal clear` before starting another in the same session.
12. The shell command emits a model-facing handoff; only the Grok agent calls `get_goal`, `create_goal`, or `update_goal` tools.
...
14. DELEGATE all code edits, test writes, fixes, and QA execution to right-sized `spawn_subagent` workers (Delegation table); you read, search, plan, integrate, and QA. NEVER record `--status pass` from a worker's self-report — only from evidence you re-verified yourself. Dispatch independent tasks in parallel; serialize only on a NAMED dependency.
...
- Grok `get_goal` reports a different active goal: checkpoint blocker, stop, surface.
